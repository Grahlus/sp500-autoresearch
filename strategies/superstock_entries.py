...
def build_superstock_breakout_entries(
    ...
    screen = screen or build_superstock_screen(data, weekly=weekly)
    ...
    rule_masks = {
        ...
        "not_too_extended_from_pivot": breakout_pct_above_pivot <= 0.10,
        "daily_volume_expansion": volume >= (avg_volume_20d_excl_today * 1.5),
        "daily_dollar_volume_expansion": (close * volume) >= (avg_dollar_volume_20d_excl_today * 1.5),
    }
...
def build_superstock_entry_weights(
    ...
    entries = entries or build_superstock_breakout_entries(data)
    close = data["close"]
