...
Exp 811: roc_240 entry threshold 0.00005 re-sweep.
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        base_long_enter = ema_fast[i] > slow + 0.05 * atr_val and roc_240_arr[i] > 0.00005 and roc_60_arr[i] > 0.0025
        ...
        if position == 1:
            if dip_tier > 0:
                ...
                if (not base_long and roc_240_arr[i] <= 0.0003 and roc_60_arr[i] <= 0.0001 and roc_15_arr[i] <= 0.00005 and roc_5_arr[i] <= 0.0001 and rsi_14_arr[i] > 40) or base_short or (close < slow - 3.6 * atr_val):
                    position = 0
