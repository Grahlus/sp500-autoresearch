...
Exp 757: add roc_60 > 0 to base_long_enter filter (alongside roc_240 > 0).
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        base_long_enter = ema_fast[i] > slow + 0.05 * atr_val and roc_240_arr[i] > 0 and roc_60_arr[i] > 0
        ...
        if position == 1:
            if dip_tier > 0:
                ...
                if (not base_long and roc_240_arr[i] <= 0.0002 and roc_60_arr[i] <= 0.0001 and roc_15_arr[i] <= 0.00005 and roc_5_arr[i] <= 0.0001 and rsi_14_arr[i] > 40) or base_short or (close < slow - 3.6 * atr_val):
                    position = 0
