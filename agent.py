...
Exp 014: EMA(10) vs EMA(240) long-only crossover.
Faster fast EMA may catch trend turns sooner than EMA(15).
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    """Long-only EMA crossover: long when ema_10 > ema_240, flat otherwise."""
    close = df["close"]
