...
Exp 1342: not_freefall roc_15[i-1] < 0.0005 (in addition to roc_5[i-1] < 0.0005).
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        not_freefall = (roc_5_arr[i] > -0.0025) and (roc_5_arr[max(0, i - 1)] < 0.0005) and (roc_5_arr[max(0, i - 2)] < 0) and (roc_15_arr[max(0, i - 1)] < 0.0005) and (roc_15_arr[max(0, i - 2)] < 0) and (roc_60_arr[max(0, i - 1)] < 0) and (roc_60_arr[max(0, i - 2)] < 0)
