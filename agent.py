...
Exp 1371: base_long_enter — remove EMA gap condition (just base_long).
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        not_freefall = (roc_5_arr[i] > -0.0025) and (roc_5_arr[max(0, i - 1)] < 0.001) and (roc_5_arr[max(0, i - 2)] < 0) and (roc_15_arr[max(0, i - 1)] < 0.001) and (roc_15_arr[max(0, i - 2)] < 0) and (roc_60_arr[max(0, i - 1)] < 0) and (roc_60_arr[max(0, i - 2)] < 0.0002)
