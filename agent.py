...
Exp 1458: not_freefall roc_60[i-2] < 0.0003 → 0.0004.
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        base_long_enter = base_long and roc_240_arr[i] > 0.0001 and roc_60_arr[i] > 0.0025 and low_vol and slow_rising10
        ...
        not_freefall = (roc_5_arr[i] > -0.0025) and (roc_5_arr[max(0, i - 1)] < 0.001) and (roc_5_arr[max(0, i - 2)] < 0) and (roc_15_arr[i] < 0.0005) and (roc_15_arr[max(0, i - 1)] < 0.001) and (roc_15_arr[max(0, i - 2)] < 0) and (roc_60_arr[i] < 0) and (roc_60_arr[max(0, i - 1)] < 0) and (roc_60_arr[max(0, i - 2)] < 0.0004) and (roc_60_arr[max(0, i - 3)] < 0.0003) and (roc_240_arr[i] < 0.002)
