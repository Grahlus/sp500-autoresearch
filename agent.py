...
Exp 1394: not_freefall — add roc_240[i] < 0.002 condition.
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        not_freefall = (roc_5_arr[i] > -0.0025) and (roc_5_arr[max(0, i - 1)] < 0.001) and (roc_5_arr[max(0, i - 2)] < 0) and (roc_15_arr[max(0, i - 1)] < 0.001) and (roc_15_arr[max(0, i - 2)] < 0) and (roc_60_arr[max(0, i - 1)] < 0) and (roc_60_arr[max(0, i - 2)] < 0.0002) and (roc_240_arr[i] < 0.002)
