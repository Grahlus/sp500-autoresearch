...
Exp 1266: not_freefall also requires roc_15[i-1] < 0 AND roc_15[i-2] < 0 (15-bar momentum declining).
"""
...
def get_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(n):
        ...
        not_freefall = (roc_5_arr[i] > -0.0025) and (roc_5_arr[max(0, i - 1)] < 0) and (roc_5_arr[max(0, i - 2)] < 0) and (roc_15_arr[max(0, i - 1)] < 0) and (roc_15_arr[max(0, i - 2)] < 0)
