...
class AnalysisAgentTests(unittest.TestCase):
    ...
    def test_analysis_report_includes_long_term_memory_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("family_summaries", report.summary["lineage"])
            self.assertIn("lineage_status_counts", report.summary["lineage"])
            self.assertIn("momentum", report.summary["lineage"]["family_summaries"])
